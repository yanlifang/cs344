...
int badChar(char* s){//check if the file has bad character 
    int i = 0;
    int badCh = 0;
    while(s[i] != '\n'){
        if(((s[i] >= 'A') && (s[i] <= 'Z')) || (s[i] == ' ')){
...
int main(int argc, char* argv[]){
    ...
    otpvalue = 1;
    setsockopt(socketFD, SOL_SOCKET, SO_REUSEADDR, &otpvalue, sizeof(int));//make socket reusable 
    //connect to server 
   ...
   // if(connect(socketFD, (struct sockaddr*)&serverAddress, sizeof(serverAddress)) < 0){//connect socket to address
    ...
    if(strcmp(buffer, "N") == 0){
        ...
        //Let's say the buffer is 100 characters, which is also the amount of data you expect.  If you read 50 in the first time,
        //then the second call to recv() would be something like recv(socket, buffer + 50, 100 - 50, 0)
        ...
        if(sizeOfText <= 1000){
            ...
            //for text and key over 1000 bytes 
            int left = sizeOfText;
            int index = 0;
            while(left > 1000){
        ...
        else{ //if size over 1000, send 1000 a time
            int left = sizeOfText;
            int index = 0;
            while(left > 1000){
        ...
        strcat(ciphertext, buffer);
        strcat(ciphertext, "\n");//add new line to end of ciphertext 
        printf("%s", ciphertext); //print ciphertext to screen 
    }
